...
RUN set -x; \
    ...
    yum install -y git-core patch bzip2 openssl-devel libyaml-devel libffi-devel readline-devel zlib-devel gdbm-devel ncurses-devel wget tar gzip gcc gcc-c++ cmake epel-release && \
...
ARG IMAGE
FROM $IMAGE
COPY --from=build /usr/lib64/libjemalloc.so.2 /usr/lib64/
